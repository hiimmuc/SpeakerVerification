...
def export_dataset_details(audio_files, os_label_dict=None, spk_label_dict=None, save_dir="dataset/train_details/"):
    print("Getting general information")
    os.makedirs(save_dir, exist_ok=True)

    for spk_folder_path in tqdm(list(audio_files), desc="Processing...", colour='red'):
        writefile = os.path.join(save_dir, f"{spk_folder_path.name}.csv")

        with open(writefile, 'w', newline='') as wf:
            spamwriter = csv.writer(wf, delimiter=',')
            header = ['ID', 'File name', 'Duration', 'Peak level dB', 'RMS level dB', 'RMS peak dB', 'RMS trough dB',
                      'Crest factor', 'Zero crossings rate', 'OS_label', 'Spk_label', 'Date', 'Source']

            spamwriter.writerow(header)

            for i, audio_file in enumerate(spk_folder_path.iterdir()):
                # general infor
                ID = str(spk_folder_path.name)
                fname = str(audio_file.name)
                source = str(Path(spk_folder_path).parent)
                date = str(audio_file).split('-')[1]
                os_label = '-' if os_label_dict is None else os_label_dict[ID]
                spk_label = '-' if spk_label_dict is None else spk_label_dict[ID]

                details = get_audio_ffmpeg_astats(fname)

                row = [audio_file.name, details['Duration'],
                       details['Peak level dB'], details['RMS level dB'], details['RMS peak dB'], details['RMS trough dB'],
                       details['Crest factor'], details['Zero crossings rate'], os_label, spk_label, date, source]
...
def update_dataset_details(root="dataset/train", save_dir="dataset/train_details/", error_file="Imposter_callbot2.txt"):
    ...
    for spk_folder_path in tqdm(list(root.iterdir())[:], desc="Processing..."):
        reading_file = os.path.join(save_dir, f"{spk_folder_path.name}.csv")
        ...
        with open(writing_file, 'w', newline='') as wf:
            ...
            for i, audio_file in enumerate(list(spk_folder_path.iterdir())):
                fp = rows[i][-1]
                row_new = rows[i]

                if isinstance(invalid_details, dict):
                    if str(root / spk_folder_path.name) in invalid_details.keys():
                        if fp in invalid_details[str(root / spk_folder_path.name)]:
                            error_rate = float(
                                invalid_details[str(root / spk_folder_path.name)][fp])
                        else:
                            error_rate = 0
                    else:
                        error_rate = 0
    ...
    if confirm_mode == 'remove':
        ...
        for spk in tqdm(invalid_spks, desc="Moving invalid speaker's dir..."):
            ...
            if os.path.exists(old_path_dir):
                subprocess.call(
                    f"mv {old_path_dir}/*.wav {new_path_dir}", shell=True)
...
if __name__ == "__main__":
    parser.add_argument('--root', type=str,
                        default="dataset/test_callbot_raw/namdp5")
    parser.add_argument('--details_dir', type=str,
                        default="dataset/details/test_cb_raw")
    ...
    if args.mode == 'export':
        ...
        update_dataset_details(
            root=args.root, save_dir=args.details_dir, error_file=args.wrong_id_file)
    elif args.mode == 'move':
