...
def get_dataset_general_infor(root="dataset/train"):
    ...
    for audio_folder in tqdm(root.iterdir(), desc="getting duration of files"):
        audio_folder_duration[audio_folder.name] = list(
            [get_duration_file(audio_file) for audio_file in audio_folder.iterdir()])

    # get size of files
    audio_folder_size = {}
    for audio_folder in tqdm(root.iterdir(), desc="getting size of files"):
        audio_folder_size[audio_folder.name] = list(
            [get_size_file(audio_file) for audio_file in audio_folder.iterdir()])
...
def get_audio_ffmpeg_astats(filename):
    ...
    """
    cmd = ['ffmpeg', '-i', filename, '-map',
           '0:a', '-af', 'astats', '-f', 'null', '-']
    ...
    details = {}
    def condition(x): return 'Parsed_astats_0' in x
    filtered_lines = list(filter(condition, output_lines))

    for line in filtered_lines:
        detail = line.replace(
            f"[{line.split('[')[-1].split(']')[0]}]", '').strip().split(':')
        if detail[0] == 'Overall':
            continue
