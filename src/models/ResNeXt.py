...
class ResNeXt(nn.Module):
    ...
    def __init__(
        ...
        self.level2 = self._make_layer(
            block, channels[2], levels[2], dsp=dsp, up_path=up_path, gate=gate)
        self.level3 = self._make_layer(
            block, channels[3], levels[3], dsp=dsp, up_path=up_path, gate=gate)
        self.level4 = self._make_layer(
            block, channels[4], levels[4], dsp=dsp, up_path=up_path, gate=gate)
        self.level5 = self._make_layer(
            block, channels[5], levels[5], dsp=dsp, up_path=up_path, gate=gate)
    ...
    def _make_layer(self, block, planes, nb_layer, dsp=False, up_path=False, gate=False):
        ...
        for l in range(nb_layer):
            layers.append(block(self.inplanes, planes,
                          dsp=dsp, up_path=up_path, gate=gate))
