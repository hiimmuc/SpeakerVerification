...
def frame_shift(features, label=None, net_pooling=None):
    if label is not None:
        ...
        for idx in range(batch_size):
            ...
            shift = - \
                abs(shift) // net_pooling if shift < 0 else shift // net_pooling
...
def mixup(features, label=None, permutation=None, c=None, alpha=0.2, beta=0.2, mixup_label_type="soft", returnc=False):
    with torch.no_grad():
        ...
        if label is not None:
            if mixup_label_type == "soft":
                mixed_label = torch.clamp(
                    c * label + (1 - c) * label[permutation, :], min=0, max=1)
            elif mixup_label_type == "hard":
                mixed_label = torch.clamp(
                    label + label[permutation, :], min=0, max=1)
            else:
...
def time_mask(features, labels=None, net_pooling=None, mask_ratios=(10, 20)):
    if labels is not None:
        ...
        t_width = torch.randint(low=int(
            n_frame/mask_ratios[1]), high=int(n_frame/mask_ratios[0]), size=(1,))   # [low, high)
...
def feature_transformation(features, n_transform, choice, filtaug_choice, filter_db_range, filter_bands,
    ...
    if n_transform == 2:
        ...
        for _ in range(n_transform):
            ...
            if choice[0]:
                if filtaug_choice == "prototype":
                    features_temp = filt_aug_prototype(
                        features_temp, db_range=filter_db_range, n_bands=filter_bands)
            ...
            if choice[1]:
                features_temp = freq_mask(
                    features_temp, mask_ratio=freq_mask_ratio)
            if choice[2]:
        ...
        if choice[0]:
            if filtaug_choice == "prototype":
                features = filt_aug_prototype(
                    features, db_range=filter_db_range, n_bands=filter_bands)
            elif filtaug_choice == "updated":
...
def filt_aug_prototype(features, db_range=(-7.5, 6), n_bands=(2, 5)):
    ...
    n_freq_band = torch.randint(
        low=n_bands[0], high=n_bands[1], size=(1,)).item()   # [low, high)
    if n_freq_band > 1:
        band_bndry_freqs = torch.cat((torch.tensor([0]),
                                      torch.sort(torch.randint(
                                          1, n_freq_bin-1, (n_freq_band - 1, )))[0],
                                      torch.tensor([n_freq_bin])))
        band_factors = torch.rand((batch_size, n_freq_band)).to(
            features) * (db_range[1] - db_range[0]) + db_range[0]
...
def filt_aug(features, db_range=[-6, 6], n_band=[3, 6], min_bw=6, filter_type="linear"):
    ...
    batch_size, n_freq_bin, _ = features.shape
    n_freq_band = torch.randint(
        low=n_band[0], high=n_band[1], size=(1,)).item()   # [low, high)
    if n_freq_band > 1:
        ...
        if filter_type == "step":
            ...
            band_factors = torch.rand(
                (batch_size, n_freq_band + 1)).to(features) * (db_range[1] - db_range[0]) + db_range[0]
            freq_filt = torch.ones((batch_size, n_freq_bin, 1)).to(features)
            for i in range(n_freq_band):
...
def add_noise(features, snrs=(15, 30), dims=(1, 2)):
    if isinstance(snrs, (list, tuple)):
        snr = (snrs[0] - snrs[1]) * torch.rand((features.shape[0],),
                                               device=features.device).reshape(-1, 1, 1) + snrs[1]
    else:
        snr = snrs
