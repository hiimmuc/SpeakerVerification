            ...
            self.apply_augmentation = aug.SomeOf(
                ...
                transforms=[
                    aug.AddBackgroundNoise(background_noise_lst,
                                           ...
                                           p=0.5),
                    aug.AddGaussianSNR(
                        min_snr_in_db=5, max_snr_in_db=40.0, p=0.5),
                    ...
                    aug.GainTransition(min_gain_in_db=-6.0,
                                       ...
                                       p=0.5,),
                    aug.PitchShift(min_semitones=-0.5,
                                   max_semitones=0.5, p=0.5),
                    ...
                    aug.Shift(min_fraction=-0.2,
                              ...
                              fade_duration=0.01,
                              p=0.5,),
                    aug.TanhDistortion(min_distortion=0.01,
                                       max_distortion=0.2, p=0.5),
                    aug.TimeMask(min_band_part=0.0,
                                 max_band_part=0.2, fade=True, p=0.5),
                    aug.TimeStretch(min_rate=0.95, max_rate=1.05,
                                    leave_length_unchanged=True, p=0.5),
                ])

    def forward(self, x):
        # batch, channel, amplitude
        if self.mode == 'torch' and len(x.shape) < 3:
            x = x.unsqueeze()
        else:
            pass
