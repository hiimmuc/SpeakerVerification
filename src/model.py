...
class SpeakerEncoder(nn.Module):
    def __init__(self, model, criterion, classifier, optimizer, features, device='cuda', gpu=0, include_top=False, ** kwargs) -> None:
        ...
        if self.features in ['mfcc', 'melspectrogram']:
            ...
            self.compute_features = Features_extractor(
        ...
        if isinstance(self.model['name'], str):
            ...
            self.__S__ = SpeakerNetModel(
                nOut=self.model['nOut'], features=self.features, device=self.device, **kwargs).to(self.device)
        else:
    ...
    def forward(self, data, label=None):
        ...
        # -> 1 x (n_speaker x batch_size) x n_samples
        data = data.reshape(-1, data.size()[-1]).unsqueeze(0).to(self.device)
        ...
        if label == None:
            ...
            feat = feat.reshape(self.nPerSpeaker, -1,
                                feat.size()[-1]).transpose(1, 0).squeeze(1)
...
class ModelHandling(object):
    def __init__(self, encoder_model,
        ...
        if self.gpu == 0:
            ...
            print(
                f"- Initialized model: {self.__model__.module.model['name']} - Trainable params: {nb_params:,}")
            print(
                f"- Using loss function: {self.__model__.module.criterion['name']}")
            print(f"- Embedding normalized: ",
                  self.__model__.module.test_normalize)
    ...
    def fit(self, loader, epoch=0, verbose=True):
        ...
        if verbose:
            loader_bar = tqdm(
                loader, desc=f">EPOCH_{epoch}", unit="it", colour="green", total=self.T_max)
        else:
            loader_bar = loader

        for (data, data_label) in loader_bar:

            # batch, channel, num_samples -> channel, batch, num_samples
    ...
    def evaluateFromList(self,
        ...
        determinator = ',' if len(lines[0].split(',')) > 1 else ' '
        if determinator == ',':
            start_index = 1
        else:
            start_index = 0
        ...
        files = list(itertools.chain(
            *[x.strip().split(determinator)[-2:] for x in lines[start_index:]]))
            ...
            for idx, line in enumerate(tqdm(lines, desc=">>>>Computing files", unit="pairs", colour="MAGENTA")):
                ...
                if cohorts_path is None:
                    ...
                    if scoring_mode == 'norm':
                        ...
                        score = similarity_measure(
                            'cosine', ref_feat, com_feat)
                    elif scoring_mode == 'pnorm':
                        score = similarity_measure(
                            'pnorm', ref_feat, com_feat, p=2)
    ...
    def testFromList(self,
        ...
        save_root = os.path.join(
            self.save_path, f"{self.__model__.module.model['name']}/{self.__model__.module.criterion['name']}/result")

        read_file = Path(test_list)
        if output_file is None:
            output_file = test_list.replace('.txt', '_result.txt')
        write_file = Path(save_root, output_file) if os.path.split(output_file)[
            0] == '' else output_file  # add parent dir if not provided
        ...
        with open(write_file, 'w', newline='') as wf:
            ...
            for idx, data in enumerate(tqdm(lines, desc=">>>>Computing files", unit="pairs", colour="MAGENTA")):
                ...
                results.append(
                    f"{str(Path(data[0]).name)},{str(Path(data[1]).name)},{score}")
        return results
    ...
    def prepare(self,
        ...
        if prepare_type == 'cohorts':
            ...
            assert isinstance(
                source, str), "Please provide path to train metadata files"
            ...
            for spkID, paths in tqdm(cohort_spk_files.items(), unit=' speakers', desc='Getting speaker embedding'):
                for path in paths[:n_emb_per_spk]:
                    emb = self.embed_utterance(
                        path, num_eval=num_eval, normalize=True)
        ...
        # Embedding preparation
            ...
            if isinstance(source, str):
                speaker_dirs = [x for x in Path(
                    source).iterdir() if x.is_dir()]
                embeds = None
                classes = {}
                # Save mean features
                for idx, speaker_dir in enumerate(speaker_dirs):
                    ...
                    if embeds is None:
                        ...
                        embeds = torch.cat(
                            (embeds, mean_embed.unsqueeze(-1)), -1)
    ...
    def export_onnx(self, state_path, check=True):
        save_root = self.save_path + \
            f"/{self.model_name}/{self.criterion}/model"
        save_path = os.path.join(
            save_root, f"model_eval_{self.model_name}.onnx")
        ...
        dim = int(self.audio_spec['sample_rate'] *
                  self.audio_spec['sentence_len'])
    ...
    def onnx_inference(self, model_path, inp):
        ...
        return onnx_output
##################################################################################################
# Use to plot model with torch summary
...
if __name__ == '__main__':
    ...
    summary(model, input_dim, device='cpu', depth=3, col_width=16, col_names=[
            "input_size", "output_size", "num_params", "mult_adds"], batch_dim=0)
