...
class MMP_Balance(torch.nn.Module):
    ...
    def forward(self, X, T):
        ...
        T_others = torch.from_numpy(numpy.delete(numpy.arange(
            self.nb_classes), T.detach().cpu().numpy())).long()
        ...
        l1 = torch.log(torch.exp(-torch.sum(out_positive *
                       new_center[T], dim=1) * self.w + self.b).sum() + 1)  # positive pair
        l2 = torch.log(torch.sum((torch.exp(F.linear(
            out_positive, new_center[T_others]) * self.w - self.b)), dim=1) + 1).mean()
        ...
        cos_sim_matrix2 = F.linear(out_anchor, P[T])
        cos_sim_matrix2 = cos_sim_matrix2 * self.w - self.b

        label = torch.from_numpy(numpy.asarray(
            range(0, X.shape[0]))).long().to(device)
        ...
        prec1 = accuracy(cos_sim_matrix2.detach(),
                         label.detach(), topk=(1,))[0]
