...
def conv_bn_act(in_, out_, kernel_size,
    ...
    return nn.Sequential(
        SamePadConv2d(in_, out_, kernel_size, stride,
...
class SEModule(nn.Module):
    def __init__(self, in_, squeeze_ch):
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv2d(in_, squeeze_ch, kernel_size=1,
                      stride=1, padding=0, bias=True),
            Swish(),
            nn.Conv2d(squeeze_ch, in_, kernel_size=1,
                      stride=1, padding=0, bias=True),
        )
...
class DropConnect(nn.Module):
    ...
    def forward(self, x):
        ...
        random_tensor += torch.rand([x.shape[0], 1, 1, 1],
